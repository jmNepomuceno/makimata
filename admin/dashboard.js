        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    // Sort: unread (sent) first, then opened, then others
                    notifications.sort((a, b) => {
                        const statusOrder = { 'sent': 1, 'opened': 2, 'delivered': 3, 'failed': 4 };
                        return (statusOrder[a.status] || 99) - (statusOrder[b.status] || 99);
                    });

                    const dropdownList = $('#notification-dropdown .dropdown-list');
                    dropdownList.empty();

                    if (notifications.length === 0) {
                        dropdownList.append('<li><p style="text-align:center; padding: 1rem;">No notifications</p></li>');
                        return;
                    }

                    notifications.forEach(notif => {
                        const statusClass = notif.status === 'sent' ? 'notif-unread' : 'notif-read';

                        const notifItem = `
                        <li class="${statusClass}">
                            <a href="#" class="dropdown-item">
                                <i class="fas ${notif.icon} icon"></i>
                                <div class="item-content">
                                    <p><strong>${notif.title}:</strong> ${notif.message}</p>
                                    <span class="item-date">${new Date(notif.created_at).toLocaleString()}</span>
                                </div>
                            </a>
                        </li>`;
                        dropdownList.append(notifItem);
                    });
